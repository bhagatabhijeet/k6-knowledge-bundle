...
description: >
  ...
  VS Code, and how to add k6 type definitions for autocomplete and inline docs.
tags:
  - k6
  - vscode
  - javascript
...
editor** to write them in. This bundle uses **Visual Studio Code (VS Code)**: free, works on
Windows, macOS and Linux, and has excellent JavaScript/TypeScript support built in.
...
k6 modules (`k6/http`, `k6/metrics`, …) are built in to k6, so VS Code doesn't know what
...
Verify it: open [hello-k6.js](/assets/code/k6-setup/hello-k6.js) in VS Code, hover over
...
Run scripts from VS Code's built-in terminal (**Ctrl+`**) — it opens in your project folder,
so `k6 run scripts/smoke-test.js` just works.
...
- **Skipping the type definitions, then thinking "IntelliSense is broken."** Without
  `@types/k6` VS Code can't know k6's built-in modules.
...
- **Opening the wrong folder.** Open the *project folder* in VS Code (File → Open Folder), not a
  single file, so the terminal and IntelliSense see `package.json`.

## Key takeaways

- k6 tests are **code**, so you need an editor; **VS Code** is the default choice.
...
- `npm install --save-dev @types/k6` adds autocomplete and hover-docs without changing
  how tests run.
- With k6 and VS Code installed, you're ready to create a project and write your first script.
